...
class  OnCacheM  {
 public:
  ...
  //Пока я читаю узел его никто не должен удалять - значит ЛИЧНО его
  //необходимо блокировать shared_lock
    ...
    */    
  OnCacheM(uint32_t   capacity) : l_capacity(capacity)  {
    init();
  }
  ...
  //TData getData(TKey const  *key)  {
  //std::shared_ptr<TData> getData(const std::shared_ptr<TKey>  &key)  {
  std::shared_ptr<TData> getData(const TKey  &key)  {
//    TONode3  *curFound  =  find(key) ;
//    if  (curFound)  {
...
//    }
  ...
  void  insertNode  (const TKey  &key,  std::shared_ptr<TData>  &&data)  {    
    ...
    const uint16_t  basketID  =  l_hash % l_capacity;
...
//    }
  ...
  TONode3  *  allocNode()  {
    ...
    if (re->key) {
      ...
      {        
        uint16_t basketID  =  re->hash % l_capacity;
  ...
  void  insertN(TONode3  *new_node,  const uint16_t basketID)  {  
      std::unique_lock<std::shared_mutex> lk(basket_locks[basketID]);
  ...
  void setOnBasketHead(TONode3  *new_node, TONode3  *cur,  uint16_t  basketID)  {
  ...
  void  setOther(TONode3  *new_node,  TONode3  *cur)  {
    ...
    void delOnBasketHead(TONode3  *nodeToDel,  TONode3  *cur,  uint16_t  basketID)  {
      if  (nodeToDel  ==  cur)  {
